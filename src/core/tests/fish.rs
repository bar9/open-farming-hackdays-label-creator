...
fn test_fish_functionality() {
    ...
    assert!(output_missing
        .validation_messages
        .contains_key("ingredients[0][fangort]"));
    let fangort_messages = output_missing
        .validation_messages
        .get("ingredients[0][fangort]")
        .unwrap();
    assert!(fangort_messages
        .iter()
        .any(|m| m == "Fangort ist erforderlich für Fisch-Zutaten."));
