...
fn bio_knospe_alle_zutaten_herkunft_conditional() {
    ...
    let input = InputBuilder::new()
        ...
        .ingredient(
            IngredientBuilder::new_agri("Rohrzucker", 700.0)
                .bio()
    ...
    assert_eq!(
        conditionals.get(keys::HERKUNFT_BENOETIGT_UEBER_50_PROZENT),
        Some(&true)
    );
}
...
fn bio_knospe_no_origin_error_when_ch_knospe_logo_shows() {
    ...
    let input = InputBuilder::new()
        ...
        .ingredient(
            IngredientBuilder::new_agri("Rohrzucker", 50.0)
                .bio()
                .origin(Country::Import)
                .build(),
        )
    ...
    assert!(
        output.validation_messages.is_empty(),
        "no origin error when the CH-Knospe shows, got: {:?}",
...
fn bio_knospe_non_agricultural_never_requires_origin() {
    ...
    let input = InputBuilder::new()
        .vollstaendig()
        .ingredient(
            IngredientBuilder::new_agri("Rohrzucker", 900.0)
                .bio()
                .origin(Country::Import)
                .build(),
        )
        .ingredient(
            IngredientBuilder::new("Dicarbonat", 5.0)
                .agricultural(false)
                .build(),
        )
        .total(905.0)
    ...
    assert!(
        !output
            .validation_messages
            .contains_key("ingredients[1][origin]"),
        "non-agricultural ingredient must not require origin, got: {:?}",
        output.validation_messages
    );
}
...
fn bio_knospe_country_display_on_label_for_all_ingredients() {
    ...
    let input = InputBuilder::new()
        .ingredient(
            IngredientBuilder::new("Milch", 300.0)
                .origin(Country::CH)
                .build(),
        )
        .ingredient(
...
fn knospe_under_90_percent_ch_namensgebende_ingredient_low_percentage_shows_origin() {
    ...
    let input = InputBuilder::new()
        ...
        .ingredient(
            IngredientBuilder::new_agri("Vanilla", 100.0)
                .origin(Country::EU)
                .namensgebend()
                .build(),
        )
...
fn knospe_under_90_validation_eggs_over_10_percent() {
    ...
    let input = InputBuilder::new()
        .vollstaendig()
        .ingredient(
            IngredientBuilder::new_agri("Hafer", 850.0)
                .origin(Country::EU)
                .build(),
        )
        .ingredient(
    ...
    assert!(messages
        .iter()
        .any(|msg| msg.contains("Eier/Honig/Fisch >10%")));
}
...
fn knospe_under_90_validation_butter_without_api_category_requires_origin() {
    ...
    assert!(butter_messages.is_some_and(|msgs| msgs.iter().any(|msg| msg
        == "Herkunftsland ist erforderlich für Milch/Fleisch/Insekten (Knospe <90% CH Regel).")));
}
...
fn knospe_under_90_validation_meat_always_requires_origin() {
    ...
    let input = InputBuilder::new()
        .vollstaendig()
        .ingredient(
            IngredientBuilder::new_agri("Hafer", 970.0)
                .origin(Country::EU)
                .build(),
        )
        .ingredient(
...
fn knospe_under_90_validation_monoproduct() {
    ...
    assert!(messages.iter().any(
        |msg| msg == "Herkunftsland ist erforderlich für Monoprodukte (Knospe <90% CH Regel)."
    ));
}
...
fn knospe_composite_parent_origin_only_on_lowest_level() {
    ...
    assert!(
        !label.contains("<b>Mehrkornmischung</b>"),
        "parent must not be bolded, got: {}",
        label
    );
    assert!(
        !label.contains("Mehrkornmischung* (CH)"),
        "parent must not show origin, got: {}",
        label
    );
    assert!(
        !label.contains("Mehrkornmischung (CH)"),
    ...
    assert!(
        label.contains("aus biologischer Landwirtschaft"),
        "bio legend missing, got: {}",
        label
    );
}
...
fn wildsammlung_marker_and_legend_over_10_percent() {
    ...
    assert!(
        output.label.contains("Wildsammlung"),
        "expected Wildsammlung legend; label: {}",
        output.label
    );
}
...
fn wildsammlung_exactly_10_percent_shows_marker() {
    ...
    assert!(
        output.label.contains('°'),
        "exactly 10% must show the ° marker; label: {}",
        output.label
    );
}
...
fn wildsammlung_under_10_percent_no_marker() {
    ...
    assert!(
        !output.label.contains('°'),
        "under 10% must NOT show a ° marker; label: {}",
        output.label
    );
}
...
fn knospe_erlaubte_ausnahme_over_5pct_blocks_logo() {
    ...
    let input = InputBuilder::new()
        .vollstaendig()
        .ingredient(
            IngredientBuilder::new_agri("Himbeeren", 600.0)
                .bio()
                .origin(Country::CH)
                .build(),
        )
    ...
    assert_eq!(
        c.get(keys::KNOSPE_ERLAUBTE_AUSNAHME_UEBER_5_PROZENT),
        Some(&true)
    );
}
...
fn knospe_erlaubte_ausnahme_exactly_5pct_keeps_logo() {
    ...
    let input = InputBuilder::new()
        .vollstaendig()
        .ingredient(
            IngredientBuilder::new_agri("Himbeeren", 950.0)
                .bio()
                .origin(Country::CH)
                .build(),
        )
...
fn knospe_erlaubte_ausnahme_bio_flag_counts_toward_the_same_cap() {
    ...
    let input = InputBuilder::new()
        ...
        .ingredient(
            IngredientBuilder::new_agri("Nonbio", 400.0)
                .origin(Country::CH)
...
fn knospe_non_agricultural_exception_does_not_count() {
    ...
    let input = InputBuilder::new()
        ...
        .ingredient(
            IngredientBuilder::new("Wasser", 400.0)
                .agricultural(false)
                .erlaubte_ausnahme_knospe()
                .build(),
        )
...
fn knospe_composite_umstellung_gets_no_bio_sachbezeichnung() {
    ...
    assert_eq!(
        c.get(keys::KNOSPE_MARKETING_ALLOWED),
        Some(&true),
        "the logo itself is unaffected"
    );
...
fn knospe_mono_umstellung_keeps_the_bio_sachbezeichnung() {
    ...
    let input = InputBuilder::new()
        .vollstaendig()
        .ingredient(
            IngredientBuilder::new_agri("Himbeeren", 1000.0)
                .bio()
                .origin(Country::CH)
                .umstellbetrieb()
                .build(),
...
fn knospe_empty_recipe_gets_no_bio_sachbezeichnung() {
    let calculator = calculator_for(crate::shared::Configuration::Knospe);
    let c = calculator
        .execute(InputBuilder::new().vollstaendig().build())
        .conditionals();
